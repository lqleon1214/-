...
public class ContactsApplication extends Application implements SharedPreferences.OnSharedPreferenceChangeListener{
    ...
    // getApiUri.
    private static final String DEFAULT_API_ROOT = PROTOCOL + "://" + HOST + ":" + PORT + SERVICE + CONTACTS;
    ...
    public Uri getApiUri(){
        synchronized (this) {
            if (null == apiRootUri) {
                ...
                // value - but it also makes the values persistent. Please
                // keep this in mind if you can the url using the variables
                // above.
                apiRootUri = Uri.parse(PreferenceManager.getDefaultSharedPreferences(this).getString(keyApiRoot,DEFAULT_API_ROOT));
            }
            return  apiRootUri;
        }
    }
