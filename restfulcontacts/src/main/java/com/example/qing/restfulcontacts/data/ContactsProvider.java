...
import com.example.qing.restfulcontacts.BuildConfig;
import com.example.qing.restfulcontacts.svc.RESTService;
import com.example.qing.restfulcontacts.utils.ColumnMap;
import com.example.qing.restfulcontacts.utils.ProjectionMap;
...
public class ContactsProvider extends ContentProvider {
    ...
    private static final ProjectionMap PROJ_MAP = new ProjectionMap.Builder()
            ...
            .addColumn(ContactsContract.Columns.STATUS,"CASE" + " WHEN " + ContactsHelper.COL_SYNC + " NOT NULL "
             + " THEN " + ContactsContract.STATUS_SYNC + " WHEN " + ContactsHelper.COL_DIRTY + " NOT NULL"
             + " THEN " + ContactsContract.STATUS_DIRTY + " ELSE "  + ContactsContract.STATUS_OK + " END").build();

    private static final ColumnMap COL_MAP = new ColumnMap.Builder().
            addColumn(ContactsContract.Columns.ID,ContactsHelper.COL_ID,ColumnMap.Type.LONG)
            .addColumn(ContactsContract.Columns.FNAME,ContactsHelper.COL_FNAME,ColumnMap.Type.STRING)
            .addColumn(ContactsContract.Columns.LNAME,ContactsHelper.COL_LNAME,ColumnMap.Type.STRING)
            .addColumn(ContactsContract.Columns.PHONE,ContactsHelper.COL_PHONE,ColumnMap.Type.STRING)
            .addColumn(ContactsContract.Columns.EMAIL,ContactsHelper.COL_EMAIL,ColumnMap.Type.STRING)
            .addColumn(ContactsContract.Columns.REMOTE_ID,ContactsHelper.COL_REMOTE_ID,ColumnMap.Type.STRING)
            .addColumn(ContactsContract.Columns.DIRTY,ContactsHelper.COL_DIRTY,ColumnMap.Type.INTEGER)
            .addColumn(ContactsContract.Columns.SYNC,ContactsHelper.COL_SYNC,ColumnMap.Type.STRING).build();
    ...
    static {
        ...
        uriMatcher.addURI(ContactsContract.AUTHORITY,ContactsContract.TABLE,CONTACTS_DIR);
        uriMatcher.addURI(ContactsContract.AUTHORITY,ContactsContract.TABLE + "/#",CONTACTS_ITEM);
    }
    ...
    public int update(Uri uri, ContentValues vals, String sel, String[] sArgs) {
        ...
        switch (uriMatcher.match(uri)) {
            case CONTACTS_DIR:
                ...
                // should be a separate virtul table...
                remoteSync = true;
                break;
            case CONTACTS_ITEM:
    ...
    public int localUpdate(Uri uri,ContentValues vals,String sel,String[] sArgs) {
        if (BuildConfig.DEBUG) {
            ...
            Log.d(TAG,"update@" + uri + "(" + buf.toString() + "):{" + vals + "}");
        }

        int updated = helper.getWritableDatabase().update(ContactsHelper.TAB_CONTACTS,vals,sel,sArgs);
        if (0 < updated) {
    ...
    // !!!
    // If you try to delete or update someting that has not yet
    ...
    private String addPkConstraint(Uri uri,String sel){
        ...
        sel = (null == sel)? pkConstraint:"(" + pkConstraint + ") AND (" + sel + ")";
        return sel;
    }
    ...
    public Uri localInsert(Uri uri,ContentValues vals) {
        ...
        long pk = helper.getWritableDatabase().insert(ContactsHelper.TAB_CONTACTS,ContactsContract.Columns.FNAME,vals);
        if (0 > pk) {
            uri = null;
        } else {
    ...
    public Cursor localQuery(SQLiteQueryBuilder qb,String[] proj,String sel,String[] selArgs,String ord) {
        return qb.query(helper.getWritableDatabase(),proj,sel,selArgs,null,null,ord);
    }
    ...
    public Cursor query(Uri uri, String[] proj, String sel, String[] selArgs, String ord) {
        ...
        qb.setTables(ContactsHelper.TAB_CONTACTS);
        qb.appendWhere("(" + ContactsHelper.COL_DELETE + " IS NULL)");
        if (0 <= pk) {
    ...
    public int delete(Uri uri, String sel, String[] sArgs) {
        ...
        vals.put(ContactsHelper.COL_DELETE,MARK);
        vals.put(ContactsHelper.COL_DIRTY,MARK);
    ...
    public int localDelete(Uri uri,String sel,String[] sArgs) {
        ...
        int updated = helper.getWritableDatabase().delete(ContactsHelper.TAB_CONTACTS,sel,sArgs);
        if (0 < updated) {
            getContext().getContentResolver().notifyChange(uri,null);
        }
        return updated;
