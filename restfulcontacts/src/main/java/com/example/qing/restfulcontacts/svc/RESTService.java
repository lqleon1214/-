...
import com.example.qing.restfulcontacts.ContactsApplication;
import com.example.qing.restfulcontacts.R;
import com.example.qing.restfulcontacts.data.ContactsContract;
import com.example.qing.restfulcontacts.data.ContactsHelper;
import com.example.qing.restfulcontacts.data.ContactsProvider;
...
public class RESTService extends IntentService {
    ...
    public static enum Op {
        ...
        static Op toOp(int code) {
            Op[] ops = Op.values();
            code = (code * -1) -1;
            return ((0 > code) || (ops.length <= code)) ? NOOP:ops[code];
        }
        int toInt(){
            return (ordinal() + 1) * -1;
        }
    ...
    public static String insert(Context ctxt, ContentValues vals) {
        Intent intent = getIntent(ctxt, Op.CREATE);
    ...
    public static String delete(Context ctxt,String id) {
        ...
        Intent intent = RESTService.getIntent(ctxt, Op.DELETE);
    ...
    public static String update(Context ctxt,String id,ContentValues vals) {
        ...
        Intent intent = RESTService.getIntent(ctxt, Op.UPDATE);
        intent.putExtra(RESTService.ID,id);
        marshalRequest(vals,intent);
        ctxt.startService(intent);
    ...
    public void onCreate() {
        super.onCreate();
        USER_AGENT = getString(R.string.app_name) + "/" + getString(R.string.app_version);
    }
    ...
    private static void marshalRequest(ContentValues vals,Intent intent){
        intent.putExtra(RESTService.FNAME,(!vals.containsKey(ContactsHelper.COL_FNAME))?"":vals.getAsString(ContactsHelper.COL_FNAME));
        intent.putExtra(RESTService.LNAME,(!vals.containsKey(ContactsHelper.COL_LNAME))?"":vals.getAsString(ContactsHelper.COL_LNAME));
        intent.putExtra(RESTService.PHONE,(!vals.containsKey(ContactsHelper.COL_PHONE))?"":vals.getAsString(ContactsHelper.COL_PHONE));
        intent.putExtra(RESTService.EMAIL,(!vals.containsKey(ContactsHelper.COL_EMAIL))?"":vals.getAsString(ContactsHelper.COL_EMAIL));
    }
    ...
    private void createContact(Bundle args) {
        ...
        try {
            ...
            sendRequest(HttpMethod.POST,uri,payload, new ResponseHandler() {
                @Override
    ...
    private void updateContact(Bundle args){
        ...
        Uri uri = ((ContactsApplication)getApplication()).getApiUri().buildUpon().appendPath(args.getString(ID)).build();
        final ContentValues vals = new ContentValues();
        try {
            String payload = new MessageHandler().marshal(args);
            sendRequest(HttpMethod.PUT, uri, payload, new ResponseHandler() {
                @Override
                public void handleResponse(BufferedReader in) throws IOException {
                    new MessageHandler().unmarshal(in,vals);
                }
    ...
    private void deleteContact(Bundle args) {
        ...
        getContentResolver().delete(ContactsContract.URI, ContactsProvider.SYNC_CONSTRAINT,new String[]{args.getString(XACT)});
    }
    ...
    private void cleanup(Bundle args,ContentValues vals) {
        ...
        if (!args.containsKey(ID)) {
            ...
            sel = new StringBuilder("(").append(ContactsProvider.SYNC_CONSTRAINT)
                    .append(") AND (").append(ContactsProvider.REMOTE_ID_CONSTRAINT).append(")")
                    .toString();
    ...
    private void checkId(Bundle args,ContentValues vals) {
        ...
        if (!id.equals(rid)) {
            Log.w(TAG,"request id does not match respoonse id: " + id + ", " + rid);
        }
        vals.remove(ContactsContract.Columns.REMOTE_ID);
    }

    // the return code is being ignored,at present
    private int sendRequest(HttpMethod method, Uri uri, String payload, ResponseHandler hdlr) throws IOException{
        if (BuildConfig.DEBUG) {
        ...
        try {
            ...
            if (null != payload) {
                ...
                conn.connect();
                Writer out = new OutputStreamWriter(new BufferedOutputStream(conn.getOutputStream()),"UTF-8");
                out.write(payload);
                out.flush();
            }
            ...
            if (null != hdlr) {
                hdlr.handleResponse(new BufferedReader(new InputStreamReader(conn.getInputStream())));
            }
        }finally {
            if (null != conn) {
                try {
