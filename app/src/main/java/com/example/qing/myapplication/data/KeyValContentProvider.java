...
public class KeyValContentProvider extends ContentProvider {
    ...
    static {
        ...
        m.put(KeyValContract.Columns.KEY,new ColumnDef(KeyValHelper.COL_KEY,ColumnDef.Type.STRING));
        m.put(KeyValContract.Columns.VAL,new ColumnDef(KeyValHelper.COL_VAL,ColumnDef.Type.STRING));
    ...
    static {
        ...
        m.put(KeyValContract.Columns.ID,KeyValHelper.COL_ID + " AS " + KeyValContract.Columns.ID);
        m.put(KeyValContract.Columns.VAL,KeyValHelper.COL_VAL + " AS " + KeyValContract.Columns.VAL);
        m.put(KeyValContract.Columns.EXTRA,"CASE WHEN " + KeyValHelper.COL_EXTRA + " NOT NULL THEN " + KeyValHelper.COL_ID + " ELSE NULL END AS " + KeyValContract.Columns.EXTRA);
        m.put(KeyValHelper.COL_EXTRA,KeyValHelper.COL_EXTRA);
        VAL_COL_AS_MAP = Collections.unmodifiableMap(m);
    }

    private static final String KEYVALS_VIRTUAL_TABLE = KeyValHelper.TAB_KEYS + " INNER JOIN " + KeyValHelper.TAB_VALS + " ON(" + KeyValHelper.COL_FK + "=" + KeyValHelper.COL_ID + ")";
    private static final String KEYVALS_PK_CONSTRAINT = KeyValHelper.TAB_KEYS + "." + KeyValHelper.COL_ROWID + "=";

    private static final Map<String,String> KEY_VAL_COL_AS_MAP;
    static {
        Map<String,String> m = new HashMap<String,String>();
        m.put(KeyValContract.Columns.ID,KeyValHelper.TAB_KEYS + "." + KeyValHelper.COL_ROWID + " AS " + KeyValContract.Columns.ID);
        m.put(KeyValContract.Columns.KEY,KeyValHelper.COL_KEY + " AS " + KeyValContract.Columns.KEY);
        m.put(KeyValContract.Columns.VAL,KeyValHelper.COL_VAL + " AS " + KeyValContract.Columns.VAL);
        m.put(KeyValContract.Columns.EXTRA,"CASE WHEN " + KeyValHelper.COL_EXTRA + " NOT NULL THEN " + KeyValHelper.COL_ID + " ELSE NULL END AS " + KeyValContract.Columns.EXTRA);
    ...
    static {
        ...
        uriMatcher.addURI(KeyValContract.AUTHORITY,KeyValContract.TABLE_VALS,STATUS_VAL_DIR);
        uriMatcher.addURI(KeyValContract.AUTHORITY,KeyValContract.TABLE_VALS + "/#",STATUS_VAL_ITEM);
        uriMatcher.addURI(KeyValContract.AUTHORITY,KeyValContract.TABLE_KEYVAL,STATUS_KEYVAL_DIR);
        uriMatcher.addURI(KeyValContract.AUTHORITY,KeyValContract.TABLE_KEYVAL + "/#",STATUS_KEYVAL_ITEM);
    }
    ...
    private long insertKeyVal(ContentValues vals) {
        ...
        try {
            db.beginTransaction();
            long id = helper.insertVal(db,vals.getAsString(KeyValContract.Columns.VAL));
            pk = helper.insertKey(db,vals.getAsString(KeyValContract.Columns.KEY),id);
    ...
    private ParcelFileDescriptor readExtras(Uri uri) throws FileNotFoundException {
    ...
    private Cursor queryKeyVals(String[] proj, String sel, String[] selArgs, String ord, long pk) {
        ...
        if (0 <= pk) {
            qb.appendWhere(KEYVALS_PK_CONSTRAINT + pk);
        }
        return qb.query(helper.getWritableDatabase(),proj,sel,selArgs,null,null,ord);
    }

    private Cursor queryVals(String[] proj, String sel, String[] selArgs, String ord, long pk) {
        SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            qb.setStrict(true);
        }
        ...
        if (0 <= pk) {
            qb.appendWhere(VALS_PK_CONSTRAINT + pk);
        }
        return qb.query(helper.getWritableDatabase(),proj,sel,selArgs,null,null,ord);
    }
