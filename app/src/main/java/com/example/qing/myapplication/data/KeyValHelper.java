...
import com.example.qing.myapplication.R;
...
public class KeyValHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + TAB_VALS + "(" + COL_ID + " integer PRIMARY KEY AUTOINCREMENT," + COL_VAL + " text," + COL_EXTRA + " text)");
        db.execSQL("CREATE TABLE " + TAB_KEYS + "(" + COL_KEY + " text UNIQUE," + COL_FK + " integer REFERENCES " + TAB_VALS + "(" + COL_ID + "))");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        try {
            db.execSQL("drop table " + TAB_VALS);
        } catch (SQLiteException e) {

        }
        try {
            db.execSQL("drop table " + TAB_KEYS);
        } catch (SQLiteException e) {
    ...
    long insertVal(SQLiteDatabase db, String val) {
        ...
        r.put(COL_VAL,val);
        long id = db.insertWithOnConflict(TAB_VALS,null,r,SQLiteDatabase.CONFLICT_IGNORE);
        return id;
    }
    ...
    void insertExtra(SQLiteDatabase db, long val, String extra) {
        ...
        db.update(TAB_VALS,r,COL_ID+"?",new String[]{String.valueOf(val)});
